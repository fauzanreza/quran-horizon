...
export default function QuranNavigator({ surahs = [], currentSurahId, onScrollToAyat }: QuranNavigatorProps) {
  ...
  // Fetching is now handled by the parent page

  // ── Keep in sync when navigating between surahs ──────────
  useEffect(() => {
    if (currentSurahId) {
  ...
  // ── Reset ayat to 1 when surah selection changes ─────────
  useEffect(() => {
    setSelectedAyatNo(1);
  }, [selectedSurahNo]);
  ...
  // ── Auto-focus search input when surah dropdown opens ────
  useEffect(() => {
    if (surahOpen) {
      setTimeout(() => surahSearchRef.current?.focus(), 60);
    } else {
      setSurahSearch("");
    }
  }, [surahOpen]);
  ...
  // ── Loading / Error states are now handled by parent page ──
    ...
    <div className="flex items-center gap-2 w-full min-w-0">
      ...
      <div ref={surahRef} className="relative flex-1 min-w-0">
        <button
          ...
          className="w-full flex items-center gap-2 rounded-2xl border border-gold-premium/30 bg-white/60 dark:bg-emerald-950/40 backdrop-blur-sm px-3 py-2.5 font-bold text-zinc-800 dark:text-zinc-100 shadow-sm hover:border-gold-premium/70 transition-all"
        ...
        >
          ...
          <span className="flex-1 text-left truncate text-xs sm:text-sm">
            {selectedSurah?.namaLatin ?? "Pilih Surah"}
          </span>
          <svg
        ...
        {surahOpen && (
          <div
            className="absolute left-0 top-full mt-2 z-[9999] w-72 rounded-2xl overflow-hidden shadow-2xl border border-gold-premium/20"
            style={{
              background: 'var(--background)',
              boxShadow: '0 20px 60px -10px rgba(6,78,59,0.2), 0 0 0 1px rgba(197,160,89,0.15)',
            }}
            role="listbox"
          >
            {/* Search */}
            ...
            {/* List */}
            <ul className="max-h-64 overflow-y-auto py-1 custom-scrollbar" role="listbox">
              {filteredSurahs.length === 0 ? (
                <li className="px-4 py-3 text-xs text-zinc-400 text-center">Tidak ditemukan</li>
              ) : (
                filteredSurahs.map((surah) => (
                  <li key={surah.nomor} role="option" aria-selected={selectedSurahNo === surah.nomor}>
                    <button
                      type="button"
                      onClick={() => {
                      ...
                      className={`w-full flex items-center gap-3 px-4 py-2.5 text-left text-sm transition-all hover:bg-gold-premium/5 ${
                        selectedSurahNo === surah.nomor ? "bg-emerald-900/5 dark:bg-emerald-400/10" : ""
                      }`}
                    >
                      <span
                        className="flex items-center justify-center h-7 w-7 shrink-0 rounded-lg text-xs font-extrabold text-white"
                        style={{
                          background: selectedSurahNo === surah.nomor
      ...
      <div ref={ayatRef} className="relative shrink-0">
        ...
        {ayatOpen && (
          <div
            className="absolute left-0 top-full mt-2 z-[9999] w-36 rounded-2xl overflow-hidden shadow-2xl border border-gold-premium/20"
            style={{
              background: 'var(--background)',
              boxShadow: '0 20px 60px -10px rgba(6,78,59,0.2), 0 0 0 1px rgba(197,160,89,0.15)',
            }}
          ...
          >
            <ul className="max-h-56 overflow-y-auto py-1 custom-scrollbar">
              {ayatNumbers.map((n) => (
                <li key={n} role="option" aria-selected={selectedAyatNo === n}>
                  <button
                    type="button"
                  ...
                  >
                    <span
                      ...
                      style={{
                        background: selectedAyatNo === n ? 'linear-gradient(135deg,#b38d45,#d4af37)' : 'rgba(197,160,89,0.1)',
